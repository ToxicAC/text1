...
class PromptGazeLLE(nn.Module):
    ...
    def forward(self, images, bboxes, target_info=None):
        ...
        # --- Training-only: compute f_vision and f_text for MCR loss ---
        if self.training and target_info is not None:
            # Global vision feature: mean-pool over 1024 spatial tokens
            f_vision = pure_vision.mean(dim=1)  # [sum(N_p), dim]
            f_vision = self.vision_proj(f_vision)  # [sum(N_p), dim]
            ...
            return heatmap_preds, inout_preds, f_vision, f_text
