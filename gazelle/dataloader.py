...
from gazelle.utils import compute_bilinear_weights
...
class GazeDataset(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Compute head center and target point for bilinear interpolation weights
        head_cx = (bbox_norm[0] + bbox_norm[2]) / 2.0
        head_cy = (bbox_norm[1] + bbox_norm[3]) / 2.0
        target_x = gazex_norm[0] if gazex_norm[0] >= 0 else 0.0
        target_y = gazey_norm[0] if gazey_norm[0] >= 0 else 0.0

        head_indices, head_weights = compute_bilinear_weights(head_cx, head_cy)
        target_indices, target_weights = compute_bilinear_weights(target_x, target_y)

        if self.split == "train":
            heatmap = utils.get_heatmap(gazex_norm[0], gazey_norm[0], 64, 64) # note for training set, there is only one annotation
            return img, bbox_norm, gazex_norm, gazey_norm, torch.tensor(inout), height, width, heatmap, head_indices, head_weights, target_indices, target_weights
        else:
            return img, bbox_norm, gazex_norm, gazey_norm, torch.tensor(inout), height, width, head_indices, head_weights, target_indices, target_weights
