...
def compute_mcr_loss(f_vision, f_text, temperature=0.07):
    """InfoNCE loss for cross-modal distillation.

    Vision global feature must match its own text feature (positive)
    while distinguishing it from text features of other samples (negatives).
    Prevents feature collapse via batch-wise contrastive learning.

    Args:
        f_vision: [N, dim] projected vision global feature
    ...
    """
    # L2 normalize
    v_norm = torch.nn.functional.normalize(f_vision, dim=1)  # [N, dim]
    t_norm = torch.nn.functional.normalize(f_text, dim=1)    # [N, dim]

    # Cosine similarity matrix [N, N]
    logits = torch.mm(v_norm, t_norm.t()) / temperature

    # Diagonal = positive pairs
    labels = torch.arange(logits.size(0), device=f_vision.device)
