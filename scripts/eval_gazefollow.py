...
from gazelle.model import get_gazelle_model, PromptGazeLLE
...
parser.add_argument("--model_name", type=str, default="gazelle_dinov2_vitl14",
                    choices=["gazelle_dinov2_vitb14", "gazelle_dinov2_vitl14",
                             "gazelle_dinov2_vitb14_inout", "gazelle_dinov2_vitl14_inout",
                             "prompt_gazelle_dinov2_vitb14", "prompt_gazelle_dinov2_vitl14",
                             "prompt_gazelle_dinov2_vitb14_inout", "prompt_gazelle_dinov2_vitl14_inout"])
parser.add_argument("--ckpt_path", type=str, default="./checkpoints/gazelle_dinov2_vitl14.pt")
parser.add_argument("--clip_model", type=str, default="ViT-B/32")
parser.add_argument("--batch_size", type=int, default=128)
args = parser.parse_args()

is_prompt = args.model_name.startswith("prompt_")
...
def main():
    ...
    if is_prompt:
        # Prompt model: text branch bypassed at inference
        backbone_name = "dinov2_vitb14" if "vitb14" in args.model_name else "dinov2_vitl14"
        inout = "inout" in args.model_name
        backbone = DinoV2Backbone(backbone_name)
        model = PromptGazeLLE(backbone, clip_model_name=args.clip_model, inout=inout)
        model.load_gazelle_state_dict(torch.load(args.ckpt_path, weights_only=True))
        transform = backbone.get_transform((448, 448))
    else:
        # Baseline GazeLLE model
    ...
    for _, (images, bboxes, gazex, gazey, height, width) in tqdm(enumerate(dataloader), desc="Evaluating", total=len(dataloader)):
        if is_prompt:
            # PromptGazeLLE: new forward signature, no target_info = pure vision
            heatmap_preds, inout_preds, _, _ = model(
                images=images.to(device),
                bboxes=bboxes,
            )
        else:
            # GazeLLE: dict-based forward
        ...
        for i in range(images.shape[0]):  # per image
            for j in range(len(bboxes[i])):  # per head
                auc = gazefollow_auc(heatmap_preds[i][j], gazex[i][j], gazey[i][j], height[i], width[i])
                avg_l2, min_l2 = gazefollow_l2(heatmap_preds[i][j], gazex[i][j], gazey[i][j])
                aucs.append(auc)
